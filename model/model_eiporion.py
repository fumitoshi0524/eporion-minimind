...
from eiporion.bitLinear import BitLinear


# Config
class EiporionConfig(PretrainedConfig):
    model_type = "eiporion"
...
class Attention(nn.Module):
    def __init__(self, config: EiporionConfig):
        super().__init__()
...
class FeedForward(nn.Module):
    def __init__(self, config: EiporionConfig, intermediate_size: int = None):
        super().__init__()
...
class EiporionBlock(nn.Module):
    def __init__(self, layer_id: int, config: EiporionConfig):
        super().__init__()
...
class EiporionModel(nn.Module):
    def __init__(self, config: EiporionConfig):
        ...
        self.layers = nn.ModuleList(
            [
                EiporionBlock(layerid, config)
                for layerid in range(self.num_hidden_layers)
            ]
        )
...
class EiporionForCausalLM(PreTrainedModel, GenerationMixin):
    config_class = EiporionConfig
    _tied_weights_keys = {"lm_head.weight": "model.embed_tokens.weight"}

    def __init__(self, config: EiporionConfig = None):
        self.config = config or EiporionConfig()
        super().__init__(self.config)
        self.model = EiporionModel(self.config)
